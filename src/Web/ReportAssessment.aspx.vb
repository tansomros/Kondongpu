...
Public Class ReportAssessment
    ...
    Protected Sub cmdExport_Click(sender As Object, e As EventArgs) Handles cmdExport.Click
        ...
        pnData.Visible = False
        If Convert.ToInt32(Request.Cookies("ROLE_ID").Value) = 2 Then
            ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), "Report", "window.open('Report/ReportViewer?R=" & Request("s") & "S&ST=" & ddlStatus.SelectedValue & "&LID=" & ddlLocation.SelectedValue & "&TYPE=" & ddlType.SelectedValue & "&BDT=" & Bdate & "&EDT=" & Edate & "&RPTTYPE=EXCEL','_blank');", True)
        Else
