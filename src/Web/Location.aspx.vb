...
Public Class Location
    ...
    Private Sub CheckStatusAssessment()
        ...
        If dt.Rows.Count > 0 Then
            ...
            If iStatus >= 1 And iStatus <= 5 Then ' Processing
                ...
                'End If
            ElseIf iStatus >= 6 And iStatus <= 8 Then 'Passed
                If Convert.ToInt32(Request.Cookies("ROLE_ID").Value) = 1 Then
                    cmdSave.Visible = False
                    cmdApprove.Visible = False
                    cmdDelete.Visible = False
    ...
    Protected Sub cmdSave_Click(sender As Object, e As EventArgs) Handles cmdSave.Click
        ...
        ctlU.User_GenLogfile(Request.Cookies("Username").Value, ACTTYPE_UPD, "Locations", "แก้ไขข้อมูลร้านยาจากคำขอ", "[LocationUID=" & hdLocationUID.Value & "]")
    ...
    Private Sub UploadFiles(ByVal Fileupload As Object, REFUID As Integer, sCode As String)
        ...
        If Fileupload.HasFiles Then
            For Each uploadedFile As HttpPostedFile In Fileupload.PostedFiles
                ...
                If ctlM.Media_GetCount(StrNull2Zero(Request("id")), StrNull2Zero(hdLocationUID.Value), sCode, REFUID) >= 4 Then
                    Exit Sub
                End If
                ...
                SEQNO = ctlM.Media_GetLastSEQ(StrNull2Zero(Request("id")), StrNull2Zero(hdLocationUID.Value), sCode, REFUID)
                ...
                ctlM.Media_Save(StrNull2Zero(Request("id")), StrNull2Zero(hdLocationUID.Value), sCode, REFUID, SEQNO, sfileName, Request.Cookies("UserID").Value)
            Next
        End If
    End Sub
    ...
    Protected Sub cmdAddPharmacist_Click(sender As Object, e As EventArgs) Handles cmdAddPharmacist.Click
        ...
        If StrNull2Zero(hdPharmacistUID.Value) = 0 Then
            ...
            ctlU.User_GenLogfile(Request.Cookies("Username").Value, ACTTYPE_ADD, "Pharmacists", "เพิ่มรายชื่อเภสัชกรประจำร้านยา :" & txtPName.Text, "เพิ่มแบบทีละคน")
        Else
            ...
            ctlU.User_GenLogfile(Request.Cookies("Username").Value, ACTTYPE_UPD, "Pharmacists", "แก้ไขชื่อเภสัชกรประจำร้านยา :" & txtPName.Text, "เพิ่มแบบทีละคน")
    ...
    Protected Sub cmdSaveSoftware_Click(sender As Object, e As EventArgs) Handles cmdSaveSoftware.Click
        ...
        If FileUploadA.HasFiles Then
            For Each uploadedFile As HttpPostedFile In FileUploadA.PostedFiles
                ...
                If ctlM.Media_GetCount(StrNull2Zero(Request("id")), StrNull2Zero(hdLocationUID.Value), "S", StrNull2Long(SoftID)) >= 4 Then
                    Exit Sub
                End If
                If fileExtname <> ".jpg" And fileExtname <> ".jpeg" And fileExtname <> ".png" Then
                    ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "MessageAlert", "openModalWarningInfo(this,'ผลการตรวจสอบ','ไฟล์รูปภาพต้องมีนามสกุล .jpg, .jpeg, .png เท่านั้น');", True)
                ...
                SEQNO = ctlM.Media_GetLastSEQ(StrNull2Zero(Request("id")), StrNull2Zero(hdLocationUID.Value), "S", SoftID)
                ...
                End If
                uploadedFile.SaveAs(System.IO.Path.Combine(UploadDirectory, sfileName))
                ctlM.Media_Save(StrNull2Zero(Request("id")), hdLocationUID.Value, "S", SoftID, SEQNO, sfileName, Request.Cookies("UserID").Value)
            Next
        End If
        hdSoftwareUID.Value = ""
        txtSname.Text = ""
