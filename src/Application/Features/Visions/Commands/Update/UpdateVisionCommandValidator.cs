...
{
    public class UpdateVisionCommandValidator : AbstractValidator<UpdateVisionCommand>
    {
        private readonly KondongpuDatabaseContext _context;
        public UpdateVisionCommandValidator(KondongpuDatabaseContext context)
        {
            _context = context;

            RuleFor(x => x.Id).NotEmpty()
                .WithMessage("Id ไม่สามารถเป็นค่าว่างได้")
                .NotNull().WithMessage("โปรดระบุ Id").MustAsync(BeExistAsync)
                .WithMessage("ไม่พบข้อมูล")
