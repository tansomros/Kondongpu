...
// Fetch buildings to construct all floors
const { data: listData, execute: fetchData } = await useApi(`/Building/GetAllBuildingList`, { method: 'GET' })

const buildingsList = computed(() => listData.value?.buildings ?? [])

const buildingsOptions = computed(() => {
  return buildingsList.value.map(b => ({
    title: b.name,
    value: b.id
  }))
})
...
const getBuildingName = buildingId => {
  const building = buildingsList.value.find(b => b.id === buildingId)
  return building ? building.name : '-'
}

const allFloors = computed(() => {
  const result = []
  buildingsList.value.forEach(b => {
    if (b.floors) {
      b.floors.forEach(f => {
        result.push({
          ...f,
...
const filteredItems = computed(() => {
  if (!searchQuery.value) return allFloors.value
  const q = searchQuery.value.toLowerCase()
  return allFloors.value.filter(i => i.name?.toLowerCase().includes(q) || i.code?.toLowerCase().includes(q))
})
...
const deleteItem = async id => {
  try {
    ...
    progressDialogFailureDescription.value = 'ไม่สามารถลบข้อมูลชั้นได้'
...
const loadHighlights = () => {
  try {
    const data = JSON.parse(sessionStorage.getItem('floors_highlights') || '[]')
    const map = new Map()
    const now = Date.now()
...
const saveHighlights = () => {
  sessionStorage.setItem('floors_highlights', JSON.stringify(Array.from(highlightedItems.value.entries())))
}
...
const saveItem = async () => {
  ...
  try {
    if (isEditMode.value) {
      // Edit mode
      await $api(`/Floors/${selectedItem.value.id}`, {
        method: 'PUT',
        body: {
      ...
      // Add mode
      const response = await $api(`/Floors`, {
        method: 'POST',
        body: {
          code: editItemCode.value,
          name: editItemName.value,
  ...
  <section>
    <VCard class="mb-6">
      <VCardItem class="pb-4">
        <VCardTitle>รายการข้อมูลชั้น</VCardTitle>
      </VCardItem>
        ...
        <VCard class="pa-2 pa-sm-10">
          ...
          <VCardText>
            <h4 class="text-h4 text-center mb-2">
              {{ isEditMode ? 'แก้ไขข้อมูลชั้น' : 'เพิ่มข้อมูลชั้น' }}
            </h4>
            <p class="text-body-1 text-center mb-6">
              {{ isEditMode ? 'แก้ไขรายละเอียดข้อมูลชั้น' : 'เพิ่มข้อมูลชั้นใหม่' }}
            </p>
            <VForm ref="refForm" @submit.prevent="saveItem">
              <VRow>
                <VCol cols="12">
                  <AppTextField v-model="editItemCode" label="รหัสชั้น" placeholder="กรอกรหัสชั้น" :rules="codeRules" autofocus />
                </VCol>
                <VCol cols="12">
                  <AppTextField v-model="editItemName" label="ชื่อชั้น" placeholder="กรอกชื่อชั้น" :rules="[requiredValidator]" />
                </VCol>
                <VCol cols="12">
                  <AppSelect v-model="editItemBuildingId" label="อาคาร/ตึก" placeholder="เลือกอาคาร/ตึก" :items="buildingsOptions" :rules="[requiredValidator]" />
                </VCol>
      ...
      <VDialog v-model="isViewDialogVisible" max-width="600">
        <VCard class="pa-2 pa-sm-10">
          ...
          <VCardText>
            <h4 class="text-h4 text-center mb-2">
              รายละเอียดข้อมูลชั้น
            </h4>
            <p class="text-body-1 text-center mb-6">
              รายละเอียดข้อมูลชั้นทั้งหมด
            </p>
            <VList v-if="viewItem" class="card-list">
              ...
              <VListItem>
                ...
                <template #append>
                  <span class="text-body-1">{{ viewItem.buildingName }}</span>
                </template>
              </VListItem>
