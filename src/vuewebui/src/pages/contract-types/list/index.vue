...
const loadHighlights = () => {
  try {
    const data = JSON.parse(sessionStorage.getItem('contract_types_highlights') || '[]')
    const map = new Map()
    const now = Date.now()
...
const saveHighlights = () => {
  sessionStorage.setItem('contract_types_highlights', JSON.stringify(Array.from(highlightedItems.value.entries())))
}
...
const { data: listData, execute: fetchData } = await useApi(`/ContractTypes/GetAllContractTypeList`, { method: 'GET' })
const items = computed(() => listData.value?.contractTypes ?? [])
...
const saveItem = async () => {
  ...
  try {
    if (isEditMode.value) {
      // Edit mode
      await $api(`/ContractTypes/${selectedItem.value.id}`, {
        method: 'PUT',
        body: {
      ...
      // Add mode
      const response = await $api(`/ContractTypes`, {
        method: 'POST',
        body: {
          name: editItemName.value
        }
...
const deleteItem = async id => {
  try {
    ...
    await $api(`/ContractTypes/${id}`, { method: 'DELETE' })
...
<template>
  <section>
    <VCard class="mb-6">
      <VCardItem class="pb-4"><VCardTitle>รายการประเภทสัญญา</VCardTitle></VCardItem>
      <VDivider />
      ...
      <VDataTableServer v-model:items-per-page="itemsPerPage" v-model:model-value="selectedRows" v-model:page="page" :items="filteredItems" item-value="id" :items-length="filteredItems.length" :headers="headers" class="text-no-wrap" show-select :row-props="getRowProps">
        ...
        <template #item.actions="{ item }">
          <!--<RouterLink :to="{ name: 'contract-types-view-id', params: { id: item.id } }">
        ...
        <VCard class="pa-2 pa-sm-10">
          ...
          <VCardText>
            <h4 class="text-h4 text-center mb-2">
              {{ isEditMode ? 'แก้ไขประเภทสัญญา' : 'เพิ่มประเภทสัญญา' }}
            </h4>
            <p class="text-body-1 text-center mb-6">
              {{ isEditMode ? 'แก้ไขข้อมูลประเภทสัญญา' : 'เพิ่มข้อมูลประเภทสัญญาใหม่' }}
            </p>
            <VForm ref="refForm" @submit.prevent="saveItem">
              <VRow>
                <VCol cols="12">
                  <AppTextField v-model="editItemName" label="ชื่อประเภทสัญญา" placeholder="กรอกชื่อประเภทสัญญา" :rules="[requiredValidator]" autofocus />
                </VCol>
