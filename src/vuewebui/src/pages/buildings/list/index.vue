...
const editItemFloorCount = ref(0)
...
const items = computed(() => listData.value?.buildings ?? [])
...
const deleteItem = async id => {
  try {
    ...
    await $api(`/Building/${id}`, { method: 'DELETE' })
    progressDialogRef.value?.setSuccess()
    fetchData()
  } catch {
    progressDialogFailureDescription.value = 'ไม่สามารถลบข้อมูลอาคารได้'
...
const loadHighlights = () => {
  try {
    const data = JSON.parse(sessionStorage.getItem('buildings_highlights') || '[]')
    const map = new Map()
    const now = Date.now()
...
const saveHighlights = () => {
  sessionStorage.setItem('buildings_highlights', JSON.stringify(Array.from(highlightedItems.value.entries())))
}
...
const openEditDialog = item => {
  ...
  editItemFloorCount.value = item.floorCount ?? 0
...
const saveItem = async () => {
  ...
  try {
    if (isEditMode.value) {
      // Edit mode
      await $api(`/Building/${selectedItem.value.id}`, {
        method: 'PUT',
        body: {
          id: selectedItem.value.id,
          code: editItemCode.value,
          name: editItemName.value,
          floorCount: parseInt(editItemFloorCount.value) || 0,
      ...
      // Add mode
      const response = await $api(`/Building`, {
        method: 'POST',
        body: {
          code: editItemCode.value,
          name: editItemName.value,
  ...
  <section>
    <VCard class="mb-6">
      <VCardItem class="pb-4">
        <VCardTitle>รายการอาคาร/ตึก</VCardTitle>
      </VCardItem>
        ...
        <VCard class="pa-2 pa-sm-10">
          ...
          <VCardText>
            <h4 class="text-h4 text-center mb-2">
              {{ isEditMode ? 'แก้ไขข้อมูลอาคาร/ตึก' : 'เพิ่มอาคาร/ตึก' }}
            </h4>
            <p class="text-body-1 text-center mb-6">
              {{ isEditMode ? 'แก้ไขรายละเอียดข้อมูลอาคาร/ตึก' : 'เพิ่มข้อมูลอาคาร/ตึกใหม่' }}
            </p>
            <VForm ref="refForm" @submit.prevent="saveItem">
              <VRow>
                <VCol cols="12">
                  <AppTextField v-model="editItemCode" label="รหัส" placeholder="กรอกรหัสอาคาร" :rules="codeRules" autofocus />
                </VCol>
                <VCol cols="12">
                  <AppTextField v-model="editItemName" label="ชื่ออาคาร/ตึก" placeholder="กรอกชื่ออาคาร/ตึก" :rules="[requiredValidator]" />
                </VCol>
                <VCol cols="12">
                  <AppTextField v-model="editItemFloorCount" label="จำนวนชั้น" placeholder="กรอกจำนวนชั้น" type="number" />
                </VCol>
                <VCol cols="12">
      ...
      <VDialog v-model="isViewDialogVisible" max-width="600">
        <VCard class="pa-2 pa-sm-10">
          ...
          <VCardText>
            <h4 class="text-h4 text-center mb-2">
              รายละเอียดข้อมูลอาคาร/ตึก
            </h4>
            <p class="text-body-1 text-center mb-6">
              รายละเอียดข้อมูลอาคาร/ตึกทั้งหมด
            </p>
            <VList v-if="viewItem" class="card-list">
              ...
              <VListItem>
                ...
                <template #append>
                  <span class="text-body-1">{{ viewItem.floorCount ?? 0 }}</span>
                </template>
              </VListItem>
