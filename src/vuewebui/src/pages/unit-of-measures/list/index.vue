...
const { data: listData, execute: fetchData } = await useApi(`/UnitOfMeasures/GetAllUnitOfMeasureList`, { method: 'GET' })

const items = computed(() => listData.value?.unitOfMeasures ?? [])
...
const deleteItem = async id => {
  try {
    ...
    await $api(`/UnitOfMeasures/${id}`, { method: 'DELETE' })
    progressDialogRef.value?.setSuccess()
    fetchData()
  } catch {
    progressDialogFailureDescription.value = 'ไม่สามารถลบข้อมูลหน่วยนับได้'
...
const loadHighlights = () => {
  try {
    const data = JSON.parse(sessionStorage.getItem('unit_of_measures_highlights') || '[]')
    const map = new Map()
    const now = Date.now()
...
const saveHighlights = () => {
  sessionStorage.setItem('unit_of_measures_highlights', JSON.stringify(Array.from(highlightedItems.value.entries())))
}
...
const saveItem = async () => {
  ...
  try {
    if (isEditMode.value) {
      // Edit mode
      await $api(`/UnitOfMeasures/${selectedItem.value.id}`, {
        method: 'PUT',
        body: {
      ...
      // Add mode
      const response = await $api(`/UnitOfMeasures`, {
        method: 'POST',
        body: {
          name: editItemName.value,
          statusFlag: editItemStatus.value
  ...
  <section>
    <VCard class="mb-6">
      <VCardItem class="pb-4">
        <VCardTitle>รายการหน่วยนับ</VCardTitle>
      </VCardItem>
        ...
        <VCard class="pa-2 pa-sm-10">
          ...
          <VCardText>
            <h4 class="text-h4 text-center mb-2">
              {{ isEditMode ? 'แก้ไขข้อมูลหน่วยนับ' : 'เพิ่มหน่วยนับ' }}
            </h4>
            <p class="text-body-1 text-center mb-6">
              {{ isEditMode ? 'แก้ไขรายละเอียดข้อมูลหน่วยนับ' : 'เพิ่มข้อมูลหน่วยนับใหม่' }}
            </p>
            <VForm ref="refForm" @submit.prevent="saveItem">
              <VRow>
                <VCol cols="12">
                  <AppTextField v-model="editItemName" label="ชื่อ" placeholder="กรอกชื่อหน่วยนับ" :rules="[requiredValidator]" autofocus />
                </VCol>
                <VCol cols="12">
      ...
      <VDialog v-model="isViewDialogVisible" max-width="600">
        <VCard class="pa-2 pa-sm-10">
          ...
          <VCardText>
            <h4 class="text-h4 text-center mb-2">
              รายละเอียดข้อมูลหน่วยนับ
            </h4>
            <p class="text-body-1 text-center mb-6">
              รายละเอียดข้อมูลหน่วยนับทั้งหมด
            </p>
