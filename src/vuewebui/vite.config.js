...
const certificateArgs = process.argv.map(arg => arg.match(/--name=(?<value>.+)/i)).filter(Boolean)[0]
const certificateName = certificateArgs ? certificateArgs.groups.value : process.env.npm_package_name

if(!certificateName) {
  console.error("Invalid Certificate Name.")
  process.exit(-1)
}
...
export default defineConfig(async () => {
  if(!fs.existsSync(certFilePath) || !fs.existsSync(keyFilePath)) {
    await new Promise(resolve => {
      spawn('dotnet', ['dev-certs', 'https', '--export-path', certFilePath, '--format', 'Pem', '--no-password'], { stdio: 'inherit' })
        .on('exit', code => {
          resolve()
          if(code) {
            process.exit(code)
